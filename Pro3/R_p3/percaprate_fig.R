...
per_cap_vec = col_per_cap(phi = 0, psi = 0, pop_vector = pop_vector,D = phy_dis_vector)
...
per_spe_vec = col_per_spe(phi = 0, psi = 0, pop_vector = pop_vector,D = phy_dis_vector)
...
phi_range = seq(0,1,0.01)
psi_range = seq(0,1,0.01)
for(i in phi_range){
  for(j in psi_range){
...
ggplot(per_cap_col_dataframe, aes(phi_axis, psi_axis )) +
  geom_tile(aes(fill = spec1), color = "white") +

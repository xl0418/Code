...
scenario_title <- rep(c('high dispersal', 'intermediate dispersal', 'low dispersal'), each = 3)
...
for (scenario_index in seq(1,7,3)) {
  for (phi_index in c(1:5)) {
    for (psi_index in c(1:6)) {
      ...
      plot.combination = rbind(c(scenario_index,phi_index,psi_index),
                               c(scenario_index+1,phi_index,psi_index),
                               c(scenario_index+2,phi_index,psi_index))
      ...
      col_labels = c(paste0('SPJC ',scenario_title[scenario_index], '\n& high interaction distance'),
                     paste0('SPJC ',scenario_title[scenario_index], '\n& intermediate interaction distance'),
      ...
      column_titles <- arrangeGrob(psi1,psi2,psi3,ncol = 3)
      ...
      ltt.sce <- grid.arrange(column_titles,g_ltt1,column_titles2,g_ltt1,g_ltt4,row_titles,ncol = 2,widths = c(20,1),heights = c(2,1,25))
      
      dir_save <- 'C:/Liang/Googlebox/Research/Project3/replicate_sim_9sces_results/newmix/'
      savefilename <- paste0(dir_save,'fullmixing',scenario_index,phi_index,psi_index,'.pdf')

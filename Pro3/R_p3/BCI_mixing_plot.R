...
for (dbh_filter in scenario_vec) {
  # BCI filtering
  if (dbh_filter == 'all') {
  } else {
    full_data <- subset(bci.full7, dbh > dbh_filter)
  }
...
col_labels = c('\ndbh > 10mm','BCI data for trees with \ndbh > 30mm', '\ndbh > 100mm') 
...
# ltt.sce <- grid.arrange(column_titles, p[[1]], p[[2]], p[[3]], p[[5]], p[[4]], ncol = 2, 

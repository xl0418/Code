...
psi24 <- textGrob(bquote(psi == .(psi_title[plot.combination[4, 2]]) ~ phi == 10^.(phi_title[plot.combination[4, 3]])),
                  gp=gpar(fontsize=mu_title_fontsize, fontface=3L))
psi25 <- textGrob(bquote(psi == .(psi_title[plot.combination[5, 2]]) ~ phi == 10^.(phi_title[plot.combination[5, 3]])),
                  gp=gpar(fontsize=mu_title_fontsize, fontface=3L))

row_titles <- arrangeGrob(phi1,phi2,phi3,phi4,phi5,ncol = 1)
column_titles1 <- arrangeGrob(psi11,psi12,psi13,psi14,psi15,ncol = 5)
column_titles2 <- arrangeGrob(psi21,psi22,psi23,psi24,psi25,ncol = 5)
...
ltt.sce <- grid.arrange(column_titles1,g_ltt1,
                        column_titles2,g_ltt1,

...
// Small random number lib.
// Hanno Hildenbrandt 2015,2016
...
// give up
#if !defined(RNDUTILS_CPU_TIMESTAMP_COUNTER)
  #define RNDUTILS_CPU_TIMESTAMP = 5489u
#endif
...
namespace rndutils {
  ...
  namespace detail {
    ...
    inline auto generate_canonical(URNG& reng)
      -> enable_if_t<!std::is_unsigned<typename URNG::result_type>::value, Real>
    {
      return std::generate_canonical<Real, Bits, URNG>(reng);
    }


    // returns high-entrophy 512 bit array for seed sequence
    inline auto make_seed_array() -> std::array<uint64_t, 8>
    {
  ...
  template <typename URNG = rndutils::default_engine>
  inline auto make_random_engine() -> URNG
  {
    auto seed_array = detail::make_seed_array();
  ...
  {
    auto seed_array = std::array<uint64_t, 8>{
        0x000021ac9bcc0a2e, 0x000041f2ea94a095,
        0x000000003c10b019, 0x2bf820b4dd7c1a8a,
        0x9901cf90a40883da, 0x5a3686b2e1de6e51,
        0x000000cc0494d228, 0x000000cc04b66740
    };
    seed_array[0] ^= static_cast<uint64_t>(seed);    
    std::seed_seq sseq(seed_array.cbegin(), seed_array.cend());
    return URNG(sseq);
  }
  ...
  {
    return detail::generate_canonical<Real, -1, URNG>(reng);
  };
  ...
  public:
    ...
    using param_type = Real;
    
    Real (min)() const { return val_; }
    Real (max)() const { return val_; }

    const_distribution() {}
    explicit const_distribution(const param_type& val) : val_(val) {}	
    ...
    template <typename URNG>
    Real operator()(URNG&) const { return val_; } 

    template <typename URNG>
    Real operator()(URNG&, param_type const& val) const { return val; } 

    friend bool operator == (distribution_type const& lhs, distribution_type const& rhs) { return lhs.val_ == rhs.val_; }
    friend bool operator != (distribution_type const& lhs, distribution_type const& rhs) { return lhs.val_ != rhs.val_; }
  ...
  public:
    ...
    template <typename URNG>
    Real operator()(URNG& reng) const { return uniform01<Real>(reng); } 

    template <typename URNG>
    Real operator()(URNG& reng, param_type const&) const { return uniform01<Real>(reng); } 

    friend bool operator == (distribution_type const&, distribution_type const&) { return true; }
    friend bool operator != (distribution_type const&, distribution_type const&) { return false; }
  ...
  struct uniform_signed_distribution
  {
    using UIntT = typename std::make_unsigned<IntT>::type;

  public:
    static_assert(std::is_signed<IntT>::value && std::is_integral<IntT>::value, "invalid template argument for uniform_signed_distribution");
    ...
    {
      p_.range = static_cast<UIntT>(Max) - static_cast<UIntT>(Min);
      p_.lo = Min;
      p_.digits = detail::msb_uint(p_.range + 1);
    }
    ...
    {
      static_assert(std::is_unsigned<typename URNG::result_type>::value, "invalid template argument for uniform_signed_distribution::operator()");
    ...
    void param(const param_type&) { p_ = param; }
    void reset() {}

    friend bool operator == (distribution_type const& lhs, distribution_type const& rhs) { return lhs.p_ == rhs.p_; }
    friend bool operator != (distribution_type const& lhs, distribution_type const& rhs) { return lhs.p_ != rhs.p_; }
  ...
  public:
    ...
    friend bool operator == (distribution_type const& lhs, distribution_type const& rhs) { return true; }
    friend bool operator != (distribution_type const& lhs, distribution_type const& rhs) { return false; }
  ...
  template <typename IntType = int, typename AllZeroPolicy = all_zero_policy_throw, typename ParamType = std::vector<double>>
  ...
  public:
    ...
    static_assert(std::is_integral<result_type>::value, "Invalid template argument for mutable_discrete_distribution");
    static_assert(std::is_floating_point<weight_type>::value, "Invalid template argument for mutable_discrete_distribution");

    result_type min() const { return result_type(0); }
    result_type max() const { return static_cast<result_type>(cdf_.size() - 1); }


    mutable_discrete_distribution() : cdf_(1, result_type(0))
    { // default ctor
    }
    ...
    {
      mutate_transform_partial(first, last, 0, [](weight_type w) { return w; });
    }

    template <typename W>
    void mutate(W const& w)
    {
      mutate_transform_partial(w.cbegin(), w.cend(), 0, [](weight_type w) { return w; });
    }

    template <typename InIt>
    void mutate_partial(InIt first, InIt last, size_t ofs)
    {
      mutate_transform_partial(first, last, ofs, [](weight_type w) { return w; });
    }

    template <typename W>
    void mutate_partial(W const& w, size_t ofs)
    {
      mutate_transform_partial(w.cbegin(), w.cend(), ofs, [](weight_type w) { return w; });
    }

    template <typename InIt, typename Fun>
    void mutate_transform(InIt first, InIt last, Fun fun)
    {
      mutate_transform_partial(first, last, 0, fun);
    }
    ...
    void mutate_transform_partial(InIt first, InIt last, size_t ofs, Fun fun)
    {
      ...
      for (size_t i=ofs; i < N; ++i) {
        auto x = static_cast<weight_type>(fun(*(first + i)));
        assert(x >= weight_type(0) && "Invalid weight for mutable_discrete_distribution");
        cdf_[i] = sum += x;
      }
      apply_all_zero_policy(sum, AllZeroPolicy{});
    }

    friend bool operator == (distribution_type const& lhs, distribution_type const& rhs) { return lhs.cdf_ == rhs.cdf_; }
    friend bool operator != (distribution_type const& lhs, distribution_type const& rhs) { return lhs.cdf_ != rhs.cdf_; }
  ...
  inline void shuffle(RndIt first, RndIt last, URNG&& reng)
  {
    ...
    for (auto i = (last-first) - 1; i > 0; --i) {
      rndutils::uniform_signed_distribution<decltype(i)> d(0,i);
  ...
  inline void shuffle_n(RndIt first, RndIt last, SizeT n, URNG&& reng)
  {
    ...
    for (diff_t i=0; i<N; ++i) {
      rndutils::uniform_signed_distribution<diff_t> d(i,R);
      swap(first[i], first[d(reng)]);
    }
  }
...
template <> inline Real generate_canonical<Real, Bits, URNG>(URNG& reng) { \
  return rndutils::detail::generate_canonical<Real, Bits, URNG>(reng); \
}


#define RNDUTILS_FAST_GENERATE_CANONICAL(URNG) \
  RNDUTILS_FAST_GENERATE_CANONICAL_REAL_BITS(float, (-1), URNG) \
  RNDUTILS_FAST_GENERATE_CANONICAL_REAL_BITS(double, (-1), URNG) \
  RNDUTILS_FAST_GENERATE_CANONICAL_REAL_BITS(long double, (-1), URNG) \

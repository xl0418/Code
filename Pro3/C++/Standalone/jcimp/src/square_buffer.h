...
namespace jc {
  ...
  public:
    ...
    square_buffer(std::initializer_list<T> list) : buf_(list)
    {
      n_ = static_cast<std::size_t>(std::sqrt(buf_.size()));
    }
  ...
  namespace detail {
    ...
    template <typename T>
    inline void blk_copy(T* dst, T const* src, int w, int h, std::size_t dstS, std::size_t dstT, std::size_t srcS, std::size_t srcT, std::size_t dstStride, std::size_t srcStride)
    {
      if (w <= 0 || h <= 0) return;

...
tens2 = 0
# # for K = 20
# if(plottype == 4) {tens1_vec = c(2,3,4,1)
# } else if(plottype == 2) {tens1_vec = c(2,6)}


# for K' = 20 group 101-139
# tens1_vec = c(1,2,3,4)

# for K' = 40 group 141-179
# tens1_vec = c(5:8)

# for K' = 20V40 group 181-219
tens1_vec = c(1:4)
...
for(tens in tens1_vec){
  ...
  for(num in c(c(a:b),g20,g40) ){
    ...
    # for mu = 0.4
    # for(num in c(1,2,3,4,5,6,7,8)){
    # file1 = paste("/Volumes/Liang/Research/data/",j,"Modeltestgroup20age",num,sep = "")
    file1 = paste("/Volumes/Liang/Research/data/cluster_Kpri_20/",j,"Modeltestgroup15age",num,sep = "")
    
    
    if(num %in% g20g){
      file1 = paste("/Volumes/Liang/Research/data/cluster_Kpri_20single/",j,"Modeltestgroup15age",num,sep = "")
    }
    if(num %in% g40g){
      file1 = paste("/Volumes/Liang/Research/data/cluster_Kpri_40single/",j,"Modeltestgroup15age",num,sep = "")
    ...
    for(i in 1:100){
      ...
      # K1 = lambda_vec*K1/(lambda_vec-mu_vec[tens1])
      K_est = lambda1*K1/(lambda1-mu1)
      K = c(K,K_est)
      lambda = c(lambda,lambda1)
      mu = c(mu, mu1)
      
      # file_sim = paste(file1,"/out",i,"sim.Rdata",sep = "")
      # load(file = file_sim)
      # N_tips1 = result$tes$Nnode
      # N_tips = c(N_tips,N_tips1)
  ...
  p_mu1 = cbind(M0, mu)
  # p_mu2 = p_mu1[ p_mu1[,2] < 20,]
  p_mu3 = matrix(p_mu1, ncol = 2)
  p_mu4 = as.data.frame(p_mu3)
  colnames(p_mu4) = c("M0", "mu")
  # 
  # M0_tips = rep(c(0,0.05,0.1,0.15,0.3,0.5,1,5),each = 100)
  # p_N_tips1 = cbind(M0_tips, N_tips)
  # # p_mu2 = p_mu1[ p_mu1[,2] < 20,]
  # p_N_tips3 = matrix(p_N_tips1, ncol = 2)
  # p_N_tips4 = as.data.frame(p_N_tips3)
  # colnames(p_N_tips4) = c("M0_tips", "N_tips")
  ...
  if(tens1 == 1){
    plotl_est[[tens1]] <-ggplot(K_quantile,aes(x=factor(M0), ymin=K[,1], lower=K[,2],middle=K[,3], upper=K[,4],ymax=K[,5],fill = factor(batch),color = factor(batch))) + geom_boxplot(stat="identity")+
      # ggplot(p_K4_1, aes(x=factor(M0), y=K)) +geom_boxplot(K_quantile, outlier.shape=NA,aes(x=M0, ymin=K[,1], lower=K[,2],middle=K[,3], upper=K[,4],ymax=K[,5]),stat = "identity")+
      xlab("")+ylab("")+ 
      ...
      # scale_x_discrete(labels=c("1020" = "0","1040" = "0"))+scale_fill_manual(values=c("red", "green","blue"))+scale_color_manual(values=c("black", "black","black"))
    ...
    # geom_text(data = K_means_1, aes(label = K, y = K + 15),angle = 45)
    ...
    plotl_est[[tens1]] <- ggplot(K_quantile,aes(x=factor(M0), ymin=K[,1], lower=K[,2],middle=K[,3], upper=K[,4],ymax=K[,5],fill = factor(batch),color = factor(batch))) + geom_boxplot(stat="identity")+
      # ggplot(p_K4_1, aes(x=factor(M0), y=K)) +geom_boxplot(outlier.shape=NA)+
      xlab("")+ylab("")+ 
      theme(axis.text.x=element_text(size = x_label_fontsize,angle=90,hjust=1,vjust=0.5),panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line.x = element_line(color="black"),
            axis.title=element_blank(),axis.text.y=element_text(size = y_label_fontsize,angle=90,hjust=0.5,vjust=0.5),plot.title = element_text(size = 16),axis.line.y = element_line(color="black"),panel.background = element_blank(),legend.position="none")+
      coord_cartesian(ylim = c(0, 80)) + scale_y_continuous(breaks = sort(c(seq(0, 80, length.out=5), c(20,40))))+
  ...
  if(tens1 == 2){
    plotl_est[[tens1]] <- ggplot(lambda_quantile,aes(x=factor(M0), ymin=lambda[,1], lower=lambda[,2],middle=lambda[,3], upper=lambda[,4],ymax=lambda[,5],fill = factor(batch),color = factor(batch))) + geom_boxplot(stat="identity")+
      # ggplot(p_lambda4_1, aes(x=factor(M0), y=lambda)) +geom_boxplot(outlier.shape=NA)+
      theme(axis.text.x=element_text(size = x_label_fontsize,angle=90,hjust=1,vjust=0.5),panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line.x = element_line(color="black"),
      ...
      # scale_x_discrete(labels=c("1020" = "0","1040" = "0"))+scale_fill_manual(values=c("red", "green","blue"))+scale_color_manual(values=c("black", "black","black"))
    scale_x_discrete(labels=c("1020" = "0","1040" = "0"))+scale_fill_manual(values=c("#9ECAE1", "#FC8D59","#FDE0EF"))+scale_color_manual(values=c("#2171B5", "#EF6548","#DE77AE"))
    # geom_text(data = lambda_means_1, aes(label = lambda, y = lambda + 0.3),angle = 45)
  }
  else{
    plotl_est[[tens1]] <- ggplot(lambda_quantile,aes(x=factor(M0), ymin=lambda[,1], lower=lambda[,2],middle=lambda[,3], upper=lambda[,4],ymax=lambda[,5],fill = factor(batch),color = factor(batch))) + geom_boxplot(stat="identity")+
      # ggplot(p_lambda4_1, aes(x=factor(M0), y=lambda)) +geom_boxplot(outlier.shape=NA)+
  ...
  if(tens1 == 3){
    plotl_est[[tens1]] <-ggplot(mu_quantile,aes(x=factor(M0), ymin=mu[,1], lower=mu[,2],middle=mu[,3], upper=mu[,4],ymax=mu[,5],fill = factor(batch),color = factor(batch))) + 
      ...
      # scale_x_discrete(labels=c("1020" = "0","1040" = "0"))+scale_fill_manual(values=c("red", "green","blue"))+scale_color_manual(values=c("black", "black","black"))
    scale_x_discrete(labels=c("1020" = "0","1040" = "0"))+scale_fill_manual(values=c("#9ECAE1", "#FC8D59","#FDE0EF"))+scale_color_manual(values=c("#2171B5", "#EF6548","#DE77AE"))
    # geom_text(data = mu_means_1, aes(label = mu, y = mu + 0.1),angle = 45)
  }
  else{
    plotl_est[[tens1]] <- ggplot(mu_quantile,aes(x=factor(M0), ymin=mu[,1], lower=mu[,2],middle=mu[,3], upper=mu[,4],ymax=mu[,5],fill = factor(batch),color = factor(batch))) + geom_boxplot(stat="identity")+
      # ggplot(p_mu4_1, aes(x=factor(M0), y=mu)) +geom_boxplot(outlier.shape=NA)+
      xlab("")+
      theme(axis.text.x=element_text(size = x_label_fontsize,angle=90,hjust=1,vjust=0.5),panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(),axis.line.x = element_line(color="black"),
            axis.title=element_blank(),axis.text.y=element_text(size = y_label_fontsize,angle=90,hjust=0.5,vjust=0.5), axis.line.y = element_line(color="black"),panel.background = element_blank(),legend.position="none")+
      ylab("")+
...
legend_plot = ggplot(mu_quantile,aes(x=factor(M0), ymin=mu[,1], lower=mu[,2],middle=mu[,3], upper=mu[,4],ymax=mu[,5],fill = factor(batch),color = factor(batch))) + geom_boxplot(stat="identity")+
  ...
  # scale_fill_manual(guide=FALSE,values=c("red", "green","blue"))+
  # scale_color_manual(name = "Scenarios",labels = c("S1", "S4", "S5"),values=c("black", "black","black"))+
  # guides(color=guide_legend(override.aes=list(fill=c("red", "green","blue"))))
  # 
  # 

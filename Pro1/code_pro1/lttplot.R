...
num_group = c(c(101:109),c(111:119),c(121:129),c(131:139))+(Scenario-1)*40
  ...
  for(num in num_group){
    ...
    for (i in 1:100) {
      ...
      if(Scenario == 2) file = paste(getwd(),j,"Modeltestgroup",age,"age",num,"/out",i,"sim.Rdata",sep = "")
      if(Scenario == 1) file = paste(getwd(),j,"Modeltestgroup",age,"age",num,"/out",i,"sim.Rdata",sep = "")
      if(Scenario == 3) file = paste(getwd(),j,"Modeltestgroup",age,"age",num,"/out",i,"sim.Rdata",sep = "")
    ...
    if(count1 %in% c(18,27,36)){
      ...
      p[[count1]] <- ggplot(df_min_max, aes(x = x, y = y)) +
        ...
        geom_polygon(data = df_025, aes( group = id), fill = "gray27", alpha = 0.8)+ylab("")+xlab(substitute(paste(mu,"=",nn),list(nn = mu_vec[count1])))+
  ...
  g_ltt2 <- arrangeGrob(sg1,sg2,sg3,sg4,ncol = 4)
  
  
  label = textGrob("Number of lineages",gp=gpar(fontsize=y_title_fontsize), rot = 90)
  g_ltt4 = arrangeGrob(grobs = p, layout_matrix = m)
  g_ltt3 <- arrangeGrob(mg1,mg2,mg3,mg4,mg5,mg6,mg7,mg8,mg9,ncol = 1)
  g_ltt5 = textGrob("Time", gp=gpar(fontsize=x_title_fontsize),rot = 0)
  g_ltt1 = textGrob("")
  g_a = textGrob("(a)")
  g_b = textGrob("(b)")
  
  grid.arrange(g_ltt1,g_ltt2,g_ltt1,label,g_ltt4,g_ltt3,g_ltt1,g_ltt5,g_ltt1,ncol = 3,widths = c(1,16,3),heights = c(1,36,1))

...
if full==1:
    ...
    file_result = files + 'smc_emp_full.npy'
else:
    files = dir_path + 'pruend_data/'
    file_result = files + 'smc_emp_pruned.npy'
...
obs_param = DVParam(gamma=0.0, a=0.5, K=K, nu=nu, r=1, theta=meantrait, Vmax=1, inittrait=meantrait, initpop=1e5,
...
params[:, 3] = np.random.uniform(0.0, 1e-12, params.shape[0])  # randomize 'nu'

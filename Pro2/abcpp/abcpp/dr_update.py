...
def dr_update(previous_bestfitted_model, propose_model, params_DR, weight_gamma_dr, weight_a_dr,weight_m_dr, weight_del_dr):
    ...
    gamma_pre_var_dr = np.sum((previous_gamma_dr - gamma_pre_mean_dr) ** 2 * weight_gamma_dr)
    a_pre_mean_dr = np.sum(previous_a_dr * weight_a_dr)
    a_pre_var_dr = np.sum((previous_a_dr - a_pre_mean_dr) ** 2 * weight_a_dr)
    m_pre_mean_dr = np.sum(previous_m_dr * weight_m_dr)
    m_pre_var_dr = np.sum((previous_m_dr - m_pre_mean_dr) ** 2 * weight_m_dr)
    del_pre_mean_dr = np.sum(previous_del_dr * weight_del_dr)
    del_pre_var_dr = np.sum((previous_del_dr - del_pre_mean_dr) ** 2 * weight_del_dr)

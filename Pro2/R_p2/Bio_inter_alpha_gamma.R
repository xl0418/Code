...
combined_plot <- ggplot(data=omega_df, aes(x=deltaz, y = value, group = species))+
  ...
  ylab('Effect on the population growth')+
  geom_line(data = gamma_df,aes(y = value ,color ='Stabilizing selection'), lwd = 2)+
  ...
  annotate("text", x = 3.1, y = 0.35, label = expression(alpha==100),size = 6)+
  annotate("text", x = 3.3, y = 1.5, label = expression(gamma==5),size = 6)

phi.plot.name = 'C:\\Liang\\Googlebox\\Research\\Project2\\BaleenWhales\\result_cluster\\results_ms_posterior\\biointer_alpha_gamma.pdf'
ggsave(combined_plot,width = 8,height = 6)
